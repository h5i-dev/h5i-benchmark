# The XBOW Validation Benchmarks

Worked solutions to the
[XBOW Validation Benchmarks](https://github.com/xbow-engineering/validation-benchmarks),
implemented using [h5i](https://github.com/h5i-dev/h5i), specifically
`h5i browser` and the `h5i websec` plugin.

See the [repository README](../../README.md) for the shared purpose and scope.
...
3. **h5i**, with the `websec` plugin installed.
   ...
   ```bash
   H5I=../h5i/target/release/h5i ./examples/xben/xben-006-sqli-allowlist.sh "$URL"
   ```

## Running the examples

```bash
./examples/xben/run-all.sh            # run every example, three benchmarks at a time
./examples/xben/run-all.sh 006 021    # run only these benchmarks
JOBS=1 ./examples/xben/run-all.sh     # run one benchmark at a time
```
...
| 026 | Deprecated Developers | `cve,path_traversal,lfi` | [`xben-026-24-apache-2450-path-traversal-rce.sh`](xben-026-24-apache-2450-path-traversal-rce.sh) |
...
| 031 | Apache CGI | `command_injection,cve,path_traversal` | [`xben-031-24-apache-cgi-rce.sh`](xben-031-24-apache-cgi-rce.sh) |
...
| 034 | Melodic Mayhem | `cve` | [`xben-034-canto-remote-file-inclusion.sh`](xben-034-canto-remote-file-inclusion.sh) |
...
| 037 | Is There Anybody Out There? - Blind | `command_injection` | [`xben-037-blind-command-injection.py`](xben-037-blind-command-injection.py) [`xben-037-blind-command-injection.sh`](xben-037-blind-command-injection.sh) |
...
| 040 | SQLi Exfiltration | `blind_sqli` | [`xben-040-blind-sqli-exfiltration.py`](xben-040-blind-sqli-exfiltration.py) [`xben-040-blind-sqli-exfiltration.sh`](xben-040-blind-sqli-exfiltration.sh) |
...
| 045 | Is There Anybody Out There? (with a twist) | `command_injection` | [`xben-045-command-injection-through-the-summary.sh`](xben-045-command-injection-through-the-summary.sh) [`xben-045-filtered-blind-injection.py`](xben-045-filtered-blind-injection.py) |
...
| 095 | Pab Users | `blind_sqli` | [`xben-095-blind-sqli-then-login.sh`](xben-095-blind-sqli-then-login.sh) [`xben-095-filtered-blind-sqli.py`](xben-095-filtered-blind-sqli.py) |
